...
async def list_chats():
    logging.info("list_chats: request user=%s", current_user.id)
...
async def get_chat(chat_id):
    try:
        logging.info("get_chat: request user=%s chat_id=%s", current_user.id, chat_id)
        ...
        if not ok:
            logging.warning("get_chat: chat not found chat_id=%s", chat_id)
            return get_data_error_result(message="Chat not found!")
        # Verify chat is valid
        if str(getattr(chat, "status", "")) != StatusEnum.VALID.value:
            logging.warning("get_chat: chat not available chat_id=%s status=%s", chat_id, getattr(chat, "status", ""))
            return get_data_error_result(message="Chat not available!")
        logging.info("get_chat: success user=%s chat_id=%s chat_name=%s", current_user.id, chat_id, getattr(chat, "name", ""))
        return get_json_result(data=_build_chat_response(chat))
    except Exception as ex:
        return server_error_response(ex)
